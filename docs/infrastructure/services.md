...
The **Secure Storage Service** manages all sensitive data persistence using `expo-secure-store`.
...
The **Notification Service** encapsulates push notification registration and local notification scheduling using `expo-notifications`.

### Files

| File                                | Responsibility                                                                            |
| ----------------------------------- | ----------------------------------------------------------------------------------------- |
| `notification-service.ts`           | Public API — registration, scheduling, and listener setup                                 |
| `notification-service.constants.ts` | Notification constants (channels, Android channel config, default behavior, trigger type) |
| `notification-service.types.ts`     | Listener callback types                                                                   |
| `notification-service.utils.ts`     | `notificationAdapter` wrapping `expo-notifications`                                       |

### Key API

- `notificationService.registerForPushNotifications()`
- `notificationService.scheduleLocalNotification(title, body, data, seconds)`
- `notificationService.addListeners({ onReceived, onResponse })`

## File Service

The **File Service** provides a small wrapper around common file operations (download, delete, info) and can optionally trigger UX feedback through toast + local notifications.

### Files

| File                        | Responsibility                                   |
| --------------------------- | ------------------------------------------------ |
| `file-service.ts`           | Public API — download/delete/info helpers        |
| `file-service.types.ts`     | Function input types (e.g. `DownloadFileParams`) |
| `file-service.utils.ts`     | `fileAdapter` wrapping `expo-file-system`        |
| `file-service.constants.ts` | File-related constants (when needed)             |
